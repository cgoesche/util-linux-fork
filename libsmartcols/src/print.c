...
static int print_data(struct libscols_table *tb, struct ul_buffer *buf)
{
	...
	char *data = NULL;
	const char *name = NULL, *desc = NULL, *uri = NULL;
	int is_last;
	...
	if (cl->description)
		desc = scols_column_get_description(cl);
	...
	if (data && *data) {
		...
		if (uri) {
			...
			/* Printing the header with a column description as tooltip */
			if (!ln && desc && *desc)
				ul_fputs_hyperlink(desc, data, tb->out);
			else
				fputs(data, tb->out);
		}
	}
